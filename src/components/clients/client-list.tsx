...
import { Contact, Monitor, Globe, LayoutGrid, Cloud, Phone } from 'lucide-react'
...
function Stat({ icon: Icon, value, title }: {
  icon: React.ElementType; value: number; title: string
}) {
  return (
    <div
      title={`${value} ${title}`}
      className={`flex items-center gap-1 text-xs tabular-nums ${value > 0 ? 'text-foreground' : 'text-muted-foreground/50'}`}
    >
      <Icon size={13} className={value > 0 ? 'text-primary' : ''} />
      <span>{value}</span>
    </div>
  )
}
      ...
      {clients.map((client) => (
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            <Stat icon={Globe}      value={client._count.dnsZones}          title="zones DNS" />
