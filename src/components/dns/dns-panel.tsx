  ...
  return (
    <div>
      {canEdit && (
        <div className="mb-4">
          <Button size="sm" onClick={() => setShowZoneForm(!showZoneForm)}>
            <Plus size={14} className="mr-1.5" /> Ajouter un domaine
          </Button>
        </div>
      )}
      ...
      {zones.length === 0 && !showZoneForm && (
        <p className="text-muted-foreground text-sm">Aucun domaine configuré.</p>
      )}

      {zones.map(zone => (
        <ZoneSection key={zone.id} zone={zone} clientId={clientId} canEdit={canEdit && zone.source === 'manual'} />
      ))}
    </div>
  )
}
